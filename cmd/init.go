...
func initCommand(args []string) error {
	...
	case "haio":
		...
		print(`wget -nc https://dl.google.com/go/go1.10.2.linux-amd64.tar.gz`)
		print(`sudo rm -rf /usr/local/go`)
		print(`sudo tar -C /usr/local -xzf go1.10.2.linux-amd64.tar.gz`)
		...
		print(`go get -t github.com/RocFang/hummingbird/...`)
		print(`gopath=` + "`" + `go env | grep ^GOPATH= | cut -d '"' -f2` + "`")
		print(`pushd "${gopath}/src/github.com/RocFang/hummingbird"`)
		print(`make haio`)
		print(`popd`)
		print(`binpath=${gopath}/src/github.com/RocFang/hummingbird/bin/hummingbird`)
	case "deb":
		prefix = "build"
		usrDirName = "usr"
		...
		print(`binpath=hummingbird`)
	}
	print(``)

	print(`sudo rm -f %s/%s/bin/hummingbird`, prefix, usrDirName)
	...
	print(`nursery_replicas = 3`)
	print(`default = yes`)
	print(``)
	print(`[storage-policy:1]`)
	print(`name = repng`)
	print(`policy_type = repng`)
	print(``)
	print(`[storage-policy:2]`)
	print(`name = swift-replication`)
	...
	print(`obfuscated_prefix = haio`)
	print(`# disable tempauth if you want to use keystone`)
	print(`# tempauth_enabled = false`)
	print(``)
	...
	print(``)
	print(`# enable the next two sections for keystone`)
	print(`# don't forget to disable tempauth above`)
	print(`# [filter:authtoken]`)
	print(`# paste.filter_factory = keystonemiddleware.auth_token:filter_factory`)
	print(`# auth_uri = http://127.0.0.1:5000/`)
	print(`# auth_url = http://127.0.0.1:35357/`)
	print(`# auth_plugin = password`)
	print(`# project_domain_id = default`)
	print(`# user_domain_id = default`)
	print(`# project_name = service`)
	print(`# username = swift`)
	print(`# password = password`)
	print(`# cache = swift.cache`)
	print(`# include_service_catalog = False`)
	print(`# delay_auth_decision = True`)
	print(`# `)
	print(`# [filter:keystoneauth]`)
	print(`# reseller_prefix = AUTH`)
	print(`# operator_roles = admin, swiftoperator`)
	print(`# reseller_admin_role = ResellerAdmin`)
	print(`# GLANCE_service_roles = service`)
	print(`# default_domain_id = default`)
	print(`# allow_names_in_acls = false`)
	print(``)
	...
	printService := func(basename string, index int) {
		...
		print(`ExecReload=/%s/bin/hummingbird systemd reload \$MAINPID`, usrDirName)
		print(`ExecStop=/%s/bin/hummingbird systemd stop \$MAINPID`, usrDirName)
	...
	if subcmd == "haio" {
		print(`sudo tee %s/%s/bin/hball >/dev/null << EOF`, prefix, usrDirName)
		print(`#!/bin/bash`)
		print(``)
		...
		print(`    sudo systemctl \$@ hummingbird-andrewd &`)
		print(`    wait`)
		print(`else`)
		print(`    hummingbird \$@ all`)
		print(`fi`)
		print(`EOF`)
		print(`sudo chmod 0755 %s/%s/bin/hball`, prefix, usrDirName)
		print(``)

		print(`sudo tee %s/%s/bin/hbmain >/dev/null << EOF`, prefix, usrDirName)
		print(`#!/bin/bash`)
		print(``)
		print(`if hash systemctl 2>/dev/null ; then`)
		print(`    sudo systemctl \$@ hummingbird-proxy &`)
		...
		print(`fi`)
		print(`EOF`)
		print(`sudo chmod 0755 %s/%s/bin/hbmain`, prefix, usrDirName)
		print(``)

		print(`sudo tee %s/%s/bin/hbrings >/dev/null << EOF`, prefix, usrDirName)
		print(`#!/bin/bash`)
		print(`set -e`)
		print(``)
		...
		print(``)
		print(`hummingbird ring object.builder create 10 6 1`)
		print(`hummingbird ring object.builder add r1z1-127.0.0.1:6010/sdb1 1`)
		print(`hummingbird ring object.builder add r1z1-127.0.0.1:6010/sdb5 1`)
		print(`hummingbird ring object.builder add r1z2-127.0.0.1:6020/sdb2 1`)
		print(`hummingbird ring object.builder add r1z2-127.0.0.1:6020/sdb6 1`)
		print(`hummingbird ring object.builder add r1z3-127.0.0.1:6030/sdb3 1`)
		print(`hummingbird ring object.builder add r1z3-127.0.0.1:6030/sdb7 1`)
		print(`hummingbird ring object.builder add r1z4-127.0.0.1:6040/sdb4 1`)
		print(`hummingbird ring object.builder add r1z4-127.0.0.1:6040/sdb8 1`)
		print(`hummingbird ring object.builder rebalance`)
		print(``)
		print(`hummingbird ring object-1.builder create 10 3 1`)
		...
		print(``)
		print(`hummingbird ring object-2.builder create 10 3 1`)
		...
		print(`EOF`)
		print(`sudo chmod 0755 %s/%s/bin/hbrings`, prefix, usrDirName)
		print(``)

		print(`sudo tee %s/%s/bin/hbreset >/dev/null << EOF`, prefix, usrDirName)
		print(`#!/bin/bash`)
		print(``)
		print(`if hash systemctl 2>/dev/null ; then`)
		...
		print(`    sudo systemctl stop hummingbird-andrewd &`)
		print(`    wait`)
		print(`else`)
		...
		print(`sudo rm -f /var/local/hummingbird/andrewd.db || true`)
		print(`sudo chown -R ${USER}: /srv/hb /var/cache/swift* /var/run/swift /var/run/hummingbird /var/local/hummingbird`)
		print(`sudo chown ${USER}:adm /var/log/hummingbird`)
		print(`if hash systemctl 2>/dev/null ; then`)
		...
		print(`fi`)
		print(`if hash systemctl 2>/dev/null ; then`)
		print(`    sudo systemctl restart syslog || true`)
		print(`else`)
		print(`    sudo service syslog restart || true`)
		print(`fi`)
		print(`EOF`)
		print(`sudo chmod 0755 %s/%s/bin/hbreset`, prefix, usrDirName)
		print(``)

		print(`sudo tee %s/%s/bin/hbmount >/dev/null << EOF`, prefix, usrDirName)
		print(`#!/bin/bash`)
		print(``)
		print(`sudo mount -o loop /srv/hb-disk /srv/hb`)
		print(`EOF`)
		print(`sudo chmod 0755 %s/%s/bin/hbmount`, prefix, usrDirName)
		print(``)

		print(`sudo tee %s/%s/bin/hblog >/dev/null << EOF`, prefix, usrDirName)
		print(`#!/bin/bash`)
		print(``)
		...
		print(`fi`)
		print(`EOF`)
		print(`sudo chmod 0755 %s/%s/bin/hblog`, prefix, usrDirName)
		print(``)

		print(`sudo tee %s/%s/bin/hbswifttests >/dev/null << EOF`, prefix, usrDirName)
		print(`#!/bin/bash`)
		print(``)
		print(`cd ~/swift/test/functional`)
		print("nosetests --exclude-test-file=`go env GOPATH`/src/github.com/RocFang/hummingbird/.swift_func_excludes")
		print(`EOF`)
	...
	print(`sudo chown %s:adm %s/var/log/hummingbird`, username, prefix)
	print(`sudo chmod 0755 %s/var`, prefix)
	print(`sudo chmod 0755 %s/var/log`, prefix)
	print(`sudo chmod 0775 %s/var/log/hummingbird`, prefix)
	...
	if subcmd != "haio" {
		...
		print(`sudo %s/%s/bin/hummingbird ring %s/etc/hummingbird/account.builder create 10 1 1`, prefix, prefix, usrDirName)
		print(`sudo %s/%s/bin/hummingbird ring %s/etc/hummingbird/account.builder add r1z1-127.0.0.1:6012/hummingbird 1`, prefix, prefix, usrDirName)
		print(`sudo %s/%s/bin/hummingbird ring %s/etc/hummingbird/account.builder rebalance`, prefix, prefix, usrDirName)
		print(`sudo %s/%s/bin/hummingbird ring %s/etc/hummingbird/container.builder create 10 1 1`, prefix, prefix, usrDirName)
		print(`sudo %s/%s/bin/hummingbird ring %s/etc/hummingbird/container.builder add r1z1-127.0.0.1:6011/hummingbird 1`, prefix, prefix, usrDirName)
		print(`sudo %s/%s/bin/hummingbird ring %s/etc/hummingbird/container.builder rebalance`, prefix, prefix, usrDirName)
		print(`sudo %s/%s/bin/hummingbird ring %s/etc/hummingbird/object.builder create 10 1 1`, prefix, prefix, usrDirName)
		print(`sudo %s/%s/bin/hummingbird ring %s/etc/hummingbird/object.builder add r1z1-127.0.0.1:6010/hummingbird 1`, prefix, prefix, usrDirName)
		print(`sudo %s/%s/bin/hummingbird ring %s/etc/hummingbird/object.builder rebalance`, prefix, prefix, usrDirName)
	...
	if subcmd == "deb" {
		...
		print(`sudo find %s/bin -type f -exec chmod 0755 {} \;`, usrDirName)
	...
	if subcmd == "haio" {
		...
		print(`sudo apt install -y python-boto python-eventlet python-google-compute-engine python-mock python-netifaces python-nose python-nose-exclude python-pastedeploy python-pbr python-pyeclib python-setuptools python-swiftclient python-unittest2 python-xattr`)
		print(`if [ "$1" != "skiptests" ] ; then`)
		print(`export BOTO_CONFIG=/dev/null`)
		print(`pushd ~/swift/test/functional`)
		print(`nosetests --exclude-test-file "${gopath}/src/github.com/RocFang/hummingbird/.swift_func_excludes"`)
		print(`popd`)
		print(`fi`)
		print(`hbmain stop`)
		print(``)
	...
	case "haio":
		...
		print(`# `)
		print(`# See https://github.com/RocFang/hummingbird/blob/master/HAIO.md for more info.`)
		print(``)
	default:
		...
		print(`# curl -i -X PUT $url/container/object -H "$token" -T /%s/bin/hummingbird ; echo`, usrDirName)
