...
func TestNewServer(t *testing.T) {
	...
	require.Nil(t, err)
	ipPort, s, logger, err := NewServer(conf, &flag.FlagSet{}, confLoader)
	require.Nil(t, err)
	server, ok := s.(*ContainerServer)
	require.True(t, ok)
	require.Equal(t, "127.0.0.2", ipPort.Ip)
	require.Equal(t, 1000, ipPort.Port)
	require.Equal(t, "", ipPort.CertFile)
	require.Equal(t, "", ipPort.KeyFile)
